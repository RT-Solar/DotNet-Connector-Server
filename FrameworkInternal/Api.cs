        ...
        public override ConnectorFacade NewInstance(ConnectorInfo connectorInfo, String config)
        {
            ...
            if (connectorInfo is LocalConnectorInfoImpl)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    LOG.Error(ex, "Failed to create new connector facade: {1}, {2}: {0}", connector, config);
    ...
    public class ManagedConnectorFacadeFactoryImpl : ConnectorFacadeFactoryImpl
    {
        ...
        public override ConnectorFacade NewInstance(APIConfiguration config)
        {
            ...
            lock (CACHE)
            {
                ...
                {
                    LOG.Trace("ConnectorFacade found in cache");
                    facade = cachedFacade;
                }
            }
            return facade;
        ...
        public override ConnectorFacade NewInstance(ConnectorInfo connectorInfo, string config)
        {
            ...
            if (null == facade)
            {
                lock (CACHE)
                {
                    ...
                    {
                        LOG.Debug("ConnectorFacade found in cache");
                    }
                }
            }
            return facade;
        ...
        public override void Dispose()
        {
            ...
            foreach (ConnectorFacade facade in CACHE.Values)
            {
                if (facade is LocalConnectorFacadeImpl)
                {
                    ...
                    {
                        LOG.Error(e, "Failed to dispose facade: {0}", facade);
                    }
                }
            }
            CACHE.Clear();
