...
namespace Org.IdentityConnectors.Test.Common
{
    ...
    public sealed class TestHelpers
    {
        ...
        private const string LoadConfigErrorMessage = @"TestHelpers: Unable to load optional XML configuration file ""{0}""";
        ...
        private static IDictionary<string, object> LoadConfigurationFromFile(string filePath)
        {
            ...
            try
            {
                ...
                {
                    LOG.Info(@"The configuration file on path ""{0}"" is not found.", filePath);
                }
            }
            catch (Exception e)
            {
                LOG.Error(e, LoadConfigErrorMessage, filePath);
            }
            return properties;
        }
        ...
        private static IDictionary<string, object> LoadConfigurationFromResource(Assembly assembly, string configResName)
        {
            ...
            try
            {
                ...
                {
                    LOG.Warn(@"The configuration resource called ""{0}"" is not found.", configResName);
                }
            }
            catch (Exception e)
            {
                LOG.Error(e, LoadConfigErrorMessage, configResName);
            }
            return properties;
        }
